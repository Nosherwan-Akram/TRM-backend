...
def TR(col_maps, row_maps, corner_maps, cols_start_end, rows_start_end):

    col_m = json.dumps(col_maps)
    f = open("col_m.json","w")
    f.write(col_m)
    f.close()

    row_m = json.dumps(row_maps)
    f = open("row_m.json","w")
    f.write(row_m)
    f.close()

    corner_m = json.dumps(corner_maps)
    f = open("corner_m.json","w")
    f.write(corner_m)
    f.close()

    cols_start_end = json.dumps(cols_start_end)
    f = open("cols_start_end.json","w")
    f.write(cols_start_end)
    f.close()

    rows_start_end = json.dumps(rows_start_end)
    f = open("rows_start_end.json","w")
    f.write(rows_start_end)
    f.close()

    process = subprocess.Popen('python3 ./HTR/src/main_copy.py', shell=True)
    ret = process.communicate()[0]
    process.wait()

    process2 = subprocess.Popen('python3 ./piping_copy.py', shell=True)
    ret2 = process2.communicate([0])
    process2.wait()

TR(col_m,row_m,corner_m,cols_start_end,rows_start_end)
htrData = pd.read_excel('output_demo.xlsx',header=None).iloc[1:,1:]
print(htrData)